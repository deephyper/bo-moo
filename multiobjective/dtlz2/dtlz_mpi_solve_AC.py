...
if rank == 0:
    ...
    with open(FILENAME, "a") as fp:
        ...
        writer.writerow(["Problem name", "method", "metric", "input vars", "objectives"]
                        + bbf_num)
        writer.writerow([f"DLTZ{PROB_NUM}", "DH AugCheb", "hypervol", str(NDIMS), str(NOBJS)]
                         + hv_vals)
        writer.writerow([f"DLTZ{PROB_NUM}", "DH AugCheb", "RMSE", str(NDIMS), str(NOBJS)]
                         + rmse_vals)
        writer.writerow([f"DLTZ{PROB_NUM}", "DH AugCheb", "num pts", str(NDIMS), str(NOBJS)]
                         + npts_vals)
